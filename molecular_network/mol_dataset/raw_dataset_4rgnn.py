...
class Raw_Dataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if '.csv' in self.raw_paths[0]:
            ...
            print('df:', df)
            s = df.iloc[:,0]
            #print(f"Start processing with {len(s)} molecules.") 
        ...
        for i,smi in enumerate(s):
            ...
            #print(f"End processing with {len(data_list)} molecules added to data_list.")
        ...
        #    torch.save(self.collate(data_list), self.processed_name)
        #else:
        #    raise ValueError("Data list is empty. Check the data loading and processing steps.")

        rg = pd.DataFrame(rg_smi_list)
        smi = pd.DataFrame(valid_smi_list)
        #label = pd.DataFrame(y_list)
        df = pd.concat([rg, smi], axis=1)
        df.columns = ['rg','smi']
