...
class RGNN(torch.nn.Module):
    ...
    def forward(self, data, rg_data):
        ...
        ### bvc
        #out =torch.mean(out, dim=0).unsqueeze(0)
        #rg_out = torch.mean(rg_out, dim=0).unsqueeze(0)

        ### bvc
        out= out.view(-1).to(device)
        linear_layer = nn.Linear(out.shape[0], 256).to(device)
        out = linear_layer(out).view(1, -1).to(device)
        rg_out = rg_out.view(-1).to(device)
        linear_layer = nn.Linear(rg_out.shape[0], 256).to(device)
        rg_out = linear_layer(rg_out).view(1, -1).to(device)
